...
export function AcademiaFormModal() {
  ...
  return (
    <Modal
      open={modalAcademiaForm}
      onClose={fecharAcademiaForm}
      title={editandoAcademia ? 'Editar academia' : 'Nova academia'}
      actions={
        <>
          <Button variant="secondary" onClick={fecharAcademiaForm}>Cancelar</Button>
          <Button variant="primary" onClick={salvar}>{editandoAcademia ? 'Salvar' : 'Cadastrar'}</Button>
        </>
      }
    >
      <div className="field">
